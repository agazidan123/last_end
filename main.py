...
async def plan_history():
    ...
    try:
        ...
        db.close()
@app.post("/favorites/")
def create_favorite_endpoint(user_id: int, type: str, name: str, location: str, db: Session = Depends(get_db)):
    return create_favorite(db=db, user_id=user_id, type=type, name=name, location=location)

@app.delete("/favorites/")
def delete_favorite_endpoint(fav_id: int, db: Session = Depends(get_db)):
    return delete_favorite(db=db, fav_id=fav_id)
